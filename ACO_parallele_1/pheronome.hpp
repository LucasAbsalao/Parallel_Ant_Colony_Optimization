...
public:
    ...
    pheronome( size_t dim, const position_t& pos_food, const position_t& pos_nest,
               int rank, int nbp, double alpha = 0.7, double beta = 0.9999)
        : m_buffer_pheronome( ),
          m_dim( dim ),
          m_stride( dim + 2 ),
        ...
        i_start = 1 + (rank * dim / nbp);
        i_end   = (rank + 1) * dim / nbp;
    ...
    unsigned long get_m_stride() const{
        return m_stride;
    }
    unsigned long get_m_dim() const { 
        return m_dim; 
    }

    std::vector< pheronome_t > m_buffer_pheronome;
private:
    ...
    std::vector< pheronome_t > m_map_of_pheronome;
